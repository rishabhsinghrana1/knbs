...
//     console.log('Fetch user data for ID:', userId)
//     // Example: setFormData({ ...dataFromAPI })
//   }, [userId])
...
//             <Button variant="contained" onClick={handleSave} sx={{ bgcolor: '#FF6B35' }}>Update</Button>
//           </Box>
//         </Paper>
//       </Box>
//     </Box>
...
export default function EditUser() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        const user = await UsersService.getUser(Number(userId))
        ...
        setFormData({
          ...
          individualSite: user.individual_site,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Call API
      await UsersService.updateUser(Number(userId), userData)
    ...
    return (
      <Box sx={{ display: 'flex', height: '100vh', overflow: 'hidden', bgcolor: '#F5F7FA' }}>
        ...
        <Box sx={{ flex: 1, p: 4 }}>
          ...
          <Button variant="outlined" onClick={() => router.push('/admin/usermanagement')}>
            Back to List
          </Button>
        </Box>
      </Box>
