...
//     const payload = {
//       user_requested: formData.userRequester,
//         user_entered: loggedInUser?.id || 1, // 👈 add
//         created_by: loggedInUser?.id || 1,  // 👈 add
//       department: formData.department,
//       project: formData.project,
//       purpose: formData.purpose,
//       status: statusValue,   // 👈 yaha status change ho raha
//       request_lines: requestLines.map((line) => ({
//         breakdown_id: line.breakdown_id,
//         debtor_code: line.debtor_code,
//         debtor_name: line.debtor_name,
...
//     showAlert(`Imprest ${statusValue} successfully`, 'success')
...
// Imprest statuses per workflow docs:
// Draft → Submitted → HODApproved/HODRejected → FinanceApproved/FinanceRejected → Completed / Cancelled
const IMPREST_STATUSES = [
  'Draft',
  'Submitted',
  'HODApproved',
  'HODRejected',
...
interface Client  { DCLink: number; Name: string }
interface Account { Master_Sub_Account: string; Description: string }
...
interface LoggedInUser {
  ...
  department: string
  department_id: number
  role_name: string  // 'Admin' | 'User' | 'Finance' | 'Finance Manager' | 'Manager' | 'Director' | 'Viewer'
  permissions: {
...
const fixedField = {
  ...
  '& input': { padding: '8px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' },
}
const tableFieldStyle = {
  width: '100%',
  '& .MuiOutlinedInput-root': { height: 42, paddingRight: '6px' },
  '& input': { padding: '8px 10px', fontSize: '14px' },
}
...
export default function EditImprestDetailsPage() {
  ...
  useEffect(() => { fetchUsers(); fetchDepartments(); fetchProjects(); fetchClients(); fetchAccounts() }, [])
  useEffect(() => { if (id) fetchImprest() }, [id])

  const fetchUsers = async () => {
    try { const r = await axios.get(`${API_BASE_URL}/users/users/`); setUsers(r.data) }
    catch { showAlert('Failed to load users', 'warning') }
  }
  const fetchDepartments = async () => {
    try { const r = await axios.get(`${API_BASE_URL}/department/departments/`); setDepartments(r.data) }
    catch { showAlert('Failed to load departments', 'warning') }
  }
  const fetchProjects = async () => {
    try { const r = await axios.get(`${API_BASE_URL}/project/projects-list/`); setProjects(r.data) }
    catch { showAlert('Failed to load projects', 'warning') }
  }
  const fetchClients = async () => {
    try { const r = await axios.get(`${API_BASE_URL}/client/clients-list/`); setClients(r.data) }
    catch { showAlert('Failed to load clients', 'warning') }
  }
  const fetchAccounts = async () => {
    try { const r = await axios.get(`${API_BASE_URL}/account/accounts-list/`); setAccounts(r.data) }
    catch { showAlert('Failed to load accounts', 'warning') }
  }
  ...
  const fetchImprest = async () => {
    try {
      ...
      setRequestLines(
        (data.request_lines || []).map((line: any, idx: number) => ({
          breakdown_id:     line.breakdown_id,
          srNo:             String(idx + 1),
          debtor_code:      line.debtor_code  || '',
          debtor_name:      line.debtor_name  || '',
  ...
  const addRow = () =>
    setRequestLines((p) => [
      ...p,
      { srNo: String(p.length + 1), debtor_code: undefined, debtor_name: '', description: '', gl_account: '', account_name: '', estimated_amount: '', approved_amount: '', spent_amount: '', supplier_no: '' },
    ])
  ...
  const handleFileDelete    = (i: number) => setNewFiles((p) => p.filter((_, idx) => idx !== i))
  ...
  const handleDeleteExistingAttachment = async (attachmentId: number) => {
    ...
    try {
      await axios.delete(`${API_BASE_URL}/imprest/attachments/${attachmentId}/`)
      setExistingAttachments((p) => p.filter((a) => a.attachment_id !== attachmentId))
      showAlert('Attachment deleted', 'success')
    } catch { showAlert('Failed to delete attachment', 'error') }
  }

  // ── Build request payload ────────────────────────────────────────────────
  const buildPayload = (statusOverride?: string) => ({
    user_requested: formData.userRequester,
    user_entered:   loggedInUser?.id || 1,
  ...
  const handleSave = async () => {
    try {
      setSaving(true)
      await axios.put(`${API_BASE_URL}/imprest/${id}/`, buildPayload())
      if (newFiles.length > 0) await uploadAttachments()
      showAlert('Imprest updated successfully!', 'success')
      setTimeout(() => router.push(`/imprest/view/${id}`), 1500)
    } catch (error: any) {
      console.error('Update failed', error)
      showAlert(error.response?.data?.detail || 'Failed to update imprest', 'error')
    } finally {
      setSaving(false)
    }
  }

  // ── Status transition (with full payload to keep data) ──────────────────
  const handleStatusUpdate = async (newStatus: string) => {
    try {
      setSaving(true)
      await axios.put(`${API_BASE_URL}/imprest/${id}/`, buildPayload(newStatus))
      setFormData((p) => ({ ...p, status: newStatus }))
      showAlert(`Status updated to ${newStatus}`, 'success')
      setTimeout(() => router.push(`/imprest/view/${id}`), 1200)
    } catch (error) {
      console.error(error)
      showAlert('Status update failed', 'error')
    } finally {
  ...
  const uploadAttachments = async () => {
    ...
    } catch { showAlert('Failed to upload attachments', 'warning') }
  }
  ...
  const getDeptId      = (d: Department) => d.id ?? d.department_id ?? 0
  const getDeptName    = (d: Department) => d.department_name ?? d.name ?? ''
  const getProjectId   = (p: Project)    => p.ProjectLink ?? p.id ?? 0
  const getProjectName = (p: Project)    => p.ProjectName ?? p.project_name ?? p.name ?? `Project ${p.id}`

  // ── Role-based action button logic ───────────────────────────────────────
  //
  // Workflow:
  //   Draft       → User clicks "Submit"      → Submitted
  //   Submitted   → HOD/Manager clicks        → HODApproved / HODRejected
  //   HODApproved → Finance Manager clicks    → FinanceApproved / FinanceRejected
  //   FinanceApproved → (payment happens)     → Completed
  //
  // Roles that can approve:
  //   Manager / Director  → HOD approval step
  //   Finance Manager     → Finance approval step
  //   Admin               → can do everything
  //   Viewer              → no actions
  //
  const role = loggedInUser?.role_name || ''
  const isAdmin          = role === 'Admin'
  const isViewer         = role === 'Viewer'
  const isHOD            = role === 'Manager' || role === 'Director' || isAdmin
  const isFinanceManager = role === 'Finance Manager' || isAdmin
  const isOwner          = loggedInUser?.id === (formData.userRequester as number)
  ...
  return (
    // FIX: overflow hidden on outer, only inner scrolls
          ...
          <Paper sx={{ p: 4, borderRadius: 2 }}>
              ...
              {formData.status && (
                <Box
                  sx={{
                    ...
                    bgcolor: statusBadgeColor[formData.status] || '#6B7280',
            ...
            <Grid container spacing={3} mb={4}>
              <Grid item xs={12} md={4}>
                <TextField
                  label="Imprest Number" size="small" fullWidth disabled
                  value={formData.imprestNumber}
                  sx={{ bgcolor: '#F9FAFB' }}
                />
              </Grid>
              ...
              <Grid item xs={12} md={4}>
                <TextField
                  select label="User Requester *" size="small" fullWidth
                  value={formData.userRequester}
                  onChange={(e) => handleFormChange('userRequester', Number(e.target.value))}
                  sx={{ bgcolor: '#F9FAFB' }}
                >
                  {users.map((u) => (
                    <MenuItem key={u.id} value={u.id}>{getUserDisplayName(u)}</MenuItem>
                  ))}
                </TextField>
              </Grid>
              ...
              <Grid item xs={12} md={4}>
                <TextField
                  label="User Entered" size="small" fullWidth
              ...
              <Grid item xs={12} md={4}>
                <TextField
                  select label="Project *" size="small" fullWidth
                  value={formData.project}
                  onChange={(e) => handleFormChange('project', Number(e.target.value))}
                  sx={{ bgcolor: '#F9FAFB' }}
                >
                  {projects.map((p) => (
                    <MenuItem key={getProjectId(p)} value={getProjectId(p)}>{getProjectName(p)}</MenuItem>
                  ))}
                </TextField>
              </Grid>
              ...
              <Grid item xs={12} md={4}>
                <TextField
                  select label="Department *" size="small" fullWidth
                  value={formData.department}
                  onChange={(e) => handleFormChange('department', Number(e.target.value))}
                  sx={{ bgcolor: '#F9FAFB' }}
                >
                  {departments.map((d) => (
                    <MenuItem key={getDeptId(d)} value={getDeptId(d)}>{getDeptName(d)}</MenuItem>
                  ))}
                </TextField>
              </Grid>
              ...
              <Grid item xs={12}>
                <TextField
                  label="Purpose *" multiline rows={3} fullWidth
                  value={formData.purpose}
                  onChange={(e) => handleFormChange('purpose', e.target.value)}
                  sx={{ bgcolor: '#F9FAFB' }}
                />
                  ...
                  {existingAttachments.map((file) => (
                    <Box
                      ...
                      sx={{
                        ...
                        display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', bgcolor: 'white',
                      }}
                    >
                      <UploadIcon sx={{ fontSize: 40, color: '#FF6B35', mb: 1 }} />
                      <Typography variant="caption" sx={{ textAlign: 'center', wordBreak: 'break-all', fontSize: '0.7rem', color: '#6B7280' }}>
                      ...
                      <IconButton
                        size="small" onClick={() => handleDeleteExistingAttachment(file.attachment_id)}
                        sx={{ position: 'absolute', top: 4, right: 4, bgcolor: 'white', boxShadow: 1, width: 20, height: 20, '&:hover': { bgcolor: '#FEE2E2' } }}
                      >
                        <CloseIcon sx={{ fontSize: 14, color: '#6B7280' }} />
                      </IconButton>
                    </Box>
            ...
            <Box sx={{ mb: 4 }}>
              ...
              {newFiles.length === 0 ? (
                <Box sx={{ border: '2px dashed #E5E7EB', borderRadius: 2, p: 4, textAlign: 'center', bgcolor: '#FAFAFA' }}>
                  <input accept="*/*" style={{ display: 'none' }} id="file-upload" type="file" multiple onChange={handleFileUpload} />
                  <label htmlFor="file-upload">
                    <Button component="span" sx={{ textTransform: 'none', color: '#6B7280', fontWeight: 500 }}>
                      <UploadIcon sx={{ mr: 1 }} /> Click to upload documents
                    </Button>
                  </label>
                </Box>
              ) : (
                <Box>
                  <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', mb: 2 }}>
                    {newFiles.map((file, index) => (
                      ...
                      >
                        ...
                        <IconButton
                          size="small" onClick={() => handleFileDelete(index)}
                          sx={{ position: 'absolute', top: 4, right: 4, bgcolor: 'white', boxShadow: 1, width: 20, height: 20, '&:hover': { bgcolor: '#FEE2E2' } }}
                        >
                          <CloseIcon sx={{ fontSize: 14, color: '#6B7280' }} />
                        </IconButton>
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <Button size="small" onClick={handleDeleteAllFiles}
                ...
                <TableBody>
                  {requestLines.map((row, i) => (
                    <TableRow key={i}>
                      ...
                      {/* Debtor Code */}
                      <TableCell>
                        <Autocomplete size="small" fullWidth options={clients}
                          filterOptions={(opts, s) => { const q = s.inputValue.toLowerCase(); return opts.filter((o) => o.DCLink.toString().includes(q) || o.Name.toLowerCase().includes(q)) }}
                          getOptionLabel={(o) => `${o.DCLink}`}
                          value={clients.find((c) => c.DCLink === row.debtor_code) || null}
                          onChange={(_, val) => { handleRowChange(i, 'debtor_code', val?.DCLink || ''); handleRowChange(i, 'debtor_name', val?.Name || '') }}
                          renderInput={(params) => <TextField {...params} placeholder="Search" sx={fixedField} />}
                        />
                      </TableCell>
                      ...
                      <TableCell>
                        <Autocomplete size="small" fullWidth options={clients}
                          filterOptions={(opts, s) => { const q = s.inputValue.toLowerCase(); return opts.filter((o) => o.Name.toLowerCase().includes(q) || o.DCLink.toString().includes(q)) }}
                          getOptionLabel={(o) => o.Name}
                          value={clients.find((c) => c.DCLink === row.debtor_code) || null}
                          onChange={(_, val) => { handleRowChange(i, 'debtor_code', val?.DCLink || ''); handleRowChange(i, 'debtor_name', val?.Name || '') }}
                          renderInput={(params) => <TextField {...params} placeholder="Search" sx={tableFieldStyle} />}
                        />
                      </TableCell>

                      {/* Description */}
                      <TableCell>
                        <TextField size="small" value={row.description}
                          onChange={(e) => handleRowChange(i, 'description', e.target.value)} sx={fixedField} />
                      </TableCell>

                      {/* GL Account */}
                      <TableCell>
                        <Autocomplete size="small" fullWidth options={accounts}
                          filterOptions={(opts, s) => { const q = s.inputValue.toLowerCase(); return opts.filter((o) => o.Master_Sub_Account.toLowerCase().includes(q) || o.Description.toLowerCase().includes(q)) }}
                          getOptionLabel={(o) => o.Master_Sub_Account}
                          value={accounts.find((a) => a.Master_Sub_Account === row.gl_account) || null}
                          onChange={(_, val) => { handleRowChange(i, 'gl_account', val?.Master_Sub_Account || ''); handleRowChange(i, 'account_name', val?.Description || '') }}
                          renderInput={(params) => <TextField {...params} placeholder="Search" sx={fixedField} />}
                        />
                      </TableCell>
                      ...
                      <TableCell>
                        <Autocomplete size="small" fullWidth options={accounts}
                          filterOptions={(opts, s) => { const q = s.inputValue.toLowerCase(); return opts.filter((o) => o.Description.toLowerCase().includes(q) || o.Master_Sub_Account.toLowerCase().includes(q)) }}
                          getOptionLabel={(o) => o.Description}
                          value={accounts.find((a) => a.Master_Sub_Account === row.gl_account) || null}
                          onChange={(_, val) => { handleRowChange(i, 'gl_account', val?.Master_Sub_Account || ''); handleRowChange(i, 'account_name', val?.Description || '') }}
                          renderInput={(params) => <TextField {...params} placeholder="Search" sx={fixedField} />}
                      ...
                      {/* Amounts */}
                      {(['estimated_amount','approved_amount','spent_amount'] as const).map((field) => (
                        <TableCell key={field}>
                      ...
                      {/* Supplier */}
                      <TableCell>
                        <TextField size="small" value={row.supplier_no}
                          onChange={(e) => handleRowChange(i, 'supplier_no', e.target.value)} sx={fixedField} />
                      </TableCell>
            ...
            {/* ─────────────────────────────────────────────────────────────────
                ...
                ─────────────────────────────────────────────────────────────────
                Draft       → owner can Save / Submit
                Submitted   → HOD (Manager/Director/Admin) can Approve/Reject
                HODApproved → Finance Manager/Admin can Approve/Reject
                Viewer      → no buttons shown
            ──────────────────────────────────────────────────────────────── */}
            {!isViewer && (
              <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2, mt: 4, flexWrap: 'wrap' }}>

                {/* DRAFT: owner (User/Admin) can save or submit */}
                {currentStatus === 'Draft' && (isOwner || isAdmin) && (
                  <>
                    <Button
                      ...
                      onClick={handleSave}
                      disabled={saving}
                      sx={{ textTransform: 'none', color: '#FF6B35', borderColor: '#FF6B35' }}
                    >
                      {saving ? 'Saving...' : 'Save Draft'}
                    </Button>
                    <Button
                      variant="contained"
                      sx={{ textTransform: 'none', bgcolor: '#FF9800', '&:hover': { bgcolor: '#e68900' } }}
                ...
                {/* SUBMITTED: HOD (Manager/Director/Admin) approves or rejects */}
                {currentStatus === 'Submitted' && isHOD && (
                  <>
                    <Button
                      variant="contained"
                      color="error"
                      sx={{ textTransform: 'none' }}
                      onClick={() => handleStatusUpdate('HODRejected')}
                      disabled={saving}
                    >
                      HOD Reject
                    </Button>
                    <Button
                      ...
                      onClick={() => handleStatusUpdate('HODApproved')}
                      disabled={saving}
                    >
                      HOD Approve
                    </Button>
                ...
                {/* HODAPPROVED: Finance Manager approves or rejects */}
                {currentStatus === 'HODApproved' && isFinanceManager && (
                  <>
                    <Button
                      variant="contained"
                      color="error"
                      sx={{ textTransform: 'none' }}
                      onClick={() => handleStatusUpdate('FinanceRejected')}
                      disabled={saving}
                    >
                      Finance Reject
                    </Button>
                    <Button
                      ...
                      onClick={() => handleStatusUpdate('FinanceApproved')}
                      disabled={saving}
                    >
                      Finance Approve
                    </Button>
                ...
                {/* FINANCEAPPROVED: Admin/Finance Manager can mark Completed */}
                {currentStatus === 'FinanceApproved' && (isAdmin || isFinanceManager) && (
                  <Button
                    variant="contained"
                    sx={{ textTransform: 'none', bgcolor: '#1976D2', '&:hover': { bgcolor: '#1565C0' } }}
                ...
                {/* HODRejected / FinanceRejected: owner can cancel */}
                {['HODRejected', 'FinanceRejected'].includes(currentStatus) && (isOwner || isAdmin) && (
                  <Button
                    variant="outlined"
                    color="error"
                    sx={{ textTransform: 'none' }}
                ...
                {/* Cancel page navigation — always visible */}
                <Button
                  variant="outlined"
                  onClick={() => router.back()}
                  sx={{ textTransform: 'none' }}
