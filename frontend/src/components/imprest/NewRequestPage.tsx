...
//         }))
//       }
// console.log(imprestData,"imprestDataimprestData");
...
export default function ImprestDetailsPage() {
  ...
  useEffect(() => {
    fetchUsers(); fetchDepartments(); fetchProjects(); fetchClients(); fetchAccounts()
  }, [])

  const fetchUsers = async () => {
    try {
      const r = await axios.get(`${API_BASE_URL}/users/users/`)
      setUsers(r.data)
  ...
  const fetchDepartments = async () => {
    try {
      const r = await axios.get(`${API_BASE_URL}/department/departments/`)
  ...
  const fetchProjects = async () => {
    try {
      const r = await axios.get(`${API_BASE_URL}/project/projects-list/`)
      setProjects(r.data)
  ...
  const fetchClients = async () => {
    try {
      const r = await axios.get(`${API_BASE_URL}/client/clients-list/`)
      setClients(r.data)
  ...
  const fetchAccounts = async () => {
    try {
      const r = await axios.get(`${API_BASE_URL}/account/accounts-list/`)
      setAccounts(r.data)
  ...
  const handleFileDelete  = (i: number) => setUploadedFiles((p) => p.filter((_, idx) => idx !== i))
  const handleDeleteAll   = ()           => setUploadedFiles([])

  // ── Lines ────────────────────────────────────────────────────────────────
  const handleAddLine = () =>
    setRequestLines((p) => [
      ...p,
      { debtor_code: undefined, debtor_name: '', description: '', gl_account: '', account_name: '', estimated_amount: '', supplier_no: '' },
    ])

  const handleDeleteLine = (i: number) => setRequestLines((p) => p.filter((_, idx) => idx !== i))

  const updateLineValue = (index: number, field: keyof RequestLine, value: string | number) =>
    setRequestLines((p) => p.map((line, i) => (i === index ? { ...line, [field]: value } : line)))
  ...
  // ── Validation ───────────────────────────────────────────────────────────
  const validateForm = () => {
    if (!userRequester)  { showAlert('Please select User Requester', 'error'); return false }
    if (!department)     { showAlert('Please select Department', 'error'); return false }
    if (!project)        { showAlert('Please select Project', 'error'); return false }
    if (!purpose.trim()) { showAlert('Please enter Purpose', 'error'); return false }
    if (requestLines.length === 0) { showAlert('Please add at least one request line', 'error'); return false }

    for (let i = 0; i < requestLines.length; i++) {
      const line = requestLines[i]
      if (!line.debtor_code)                           { showAlert(`Select Client for line ${i + 1}`, 'error'); return false }
      if (!line.description.trim())                    { showAlert(`Enter Description for line ${i + 1}`, 'error'); return false }
      if (!line.estimated_amount || parseFloat(line.estimated_amount) <= 0) {
        showAlert(`Enter valid Amount for line ${i + 1}`, 'error'); return false
      }
    }
    return true
  }

  // ── Submit (status = Submitted per workflow) ─────────────────────────────
  // Workflow: User creates → clicks Submit → status becomes "Submitted"
  // Draft = saved but not submitted yet
  const handleSaveDraft = () => submitForm('Draft')
  const handleSubmit    = () => { if (validateForm()) submitForm('Submitted') }
    ...
    try {
      const payload = {
        ...
        request_lines: requestLines.map((line) => ({
          ...
          description:      line.description,
          gl_account:       line.gl_account || null,
          expense_head:     line.gl_account || null,
          account_name:     line.account_name || null,
          estimated_amount: parseFloat(line.estimated_amount),
          supplier_no:      line.supplier_no || null,
          created_by:       loggedInUser?.id || 1,
      ...
      const res  = await axios.post(`${API_BASE_URL}/imprest/`, payload)
      const created = res.data

      if (uploadedFiles.length > 0) await uploadAttachments(created.imprest_id)
      ...
      setTimeout(() => router.push('/imprest'), 1500)
    } catch (error: any) {
      console.error('Error creating imprest:', error)
      showAlert(
        error.response?.data?.detail || error.response?.data?.message || 'Failed to create imprest',
        'error'
      )
    } finally {
      setSubmitLoading(false)
  ...
  const uploadAttachments = async (imprestId: number) => {
    try {
      ...
      await axios.post(`${API_BASE_URL}/imprest/${imprestId}/upload-multiple-attachments/`, fd)
    } catch (error) {
      console.error('Attachment upload error:', error)
      showAlert('Failed to upload attachments', 'warning')
    }
  }

  const handleCancel = () => {
    if (window.confirm('Cancel? All unsaved changes will be lost.')) router.push('/imprest')
  }
  ...
  const getDeptId   = (d: Department) => d.department_id ?? d.id ?? 0
  ...
  // Total estimated
  const totalEstimated = requestLines.reduce(
    (sum, line) => sum + (parseFloat(line.estimated_amount) || 0), 0
  )
        ...
        <Box sx={{ flexGrow: 1, overflow: 'auto' }}>
          <Box sx={{ p: 3 }}>
            ...
            <Breadcrumbs separator={<NavigateNextIcon fontSize="small" />} sx={{ mb: 2 }}>
              <Link
                underline="hover"
                sx={{ cursor: 'pointer', fontSize: '0.875rem', color: '#6B7280' }}
                onClick={() => router.push('/imprest')}
              >
                Imprest
              </Link>
              <Typography fontSize="0.875rem">New Request</Typography>
            </Breadcrumbs>
            ...
            <Paper elevation={0} sx={{ p: 4, borderRadius: 2, bgcolor: 'white' }}>
              ...
              <Grid container spacing={2.5} mb={4}>
                ...
                <Grid item xs={12} md={4}>
                  ...
                  <TextField
                    fullWidth size="small" placeholder="Auto-generated" disabled
                    sx={{ bgcolor: '#F9FAFB', '& .MuiOutlinedInput-root': { '& fieldset': { borderColor: '#E5E7EB' } } }}
                  />
                </Grid>
                ...
                {/* User Entered — auto from logged-in */}
                <Grid item xs={12} md={4}>
                  <Typography variant="caption" sx={{ display: 'block', mb: 1, color: '#6B7280', fontWeight: 500 }}>
                    User Entered
                  </Typography>
                ...
                <Grid item xs={12} md={4}>
                  ...
                  >
                    {departments.map((d) => (
                      <MenuItem key={getDeptId(d)} value={getDeptId(d)}>{d.department_name}</MenuItem>
                    ))}
                  </TextField>
                </Grid>
              ...
              <Box sx={{ mb: 4 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 1 }}>
                    <input type="file" accept=".csv" hidden id="import-lines" onChange={handleImportLines} />
                    <label htmlFor="import-lines">
                ...
                <TableContainer sx={{ border: '1px solid #E5E7EB', borderRadius: 2, overflowX: 'auto' }}>
                  <Table sx={{ minWidth: 1200 }}>
                    <TableHead sx={{ bgcolor: '#F9FAFB' }}>
                      <TableRow>
                        {['Sr.', 'Debtor Code', 'Debtor Name', 'Description', 'GL Account', 'Account Name', 'Amount *', 'Supplier No.', 'Action'].map((h) => (
                          <TableCell key={h} sx={{ fontWeight: 600, color: '#6B7280', fontSize: '0.75rem', py: 1.5 }}>{h}</TableCell>
                        ))}
                      </TableRow>
                    </TableHead>
                      ...
                      {requestLines.length === 0 ? (
                        <TableRow>
                          <TableCell colSpan={9} sx={{ textAlign: 'center', py: 8 }}>
                            ...
                            <Button
                              variant="contained" size="small" onClick={handleAddLine}
                              sx={{ textTransform: 'none', bgcolor: '#FF6B35', '&:hover': { bgcolor: '#FF5722' } }}
                            >
                              + Add Line
                            </Button>
                          </TableCell>
                        ...
                        <>
                          {requestLines.map((line, index) => (
                            <TableRow key={index}>
                              ...
                              <TableCell>
                                <Autocomplete
                                  size="small" fullWidth options={clients}
                                  filterOptions={(opts, s) => {
                                    const q = s.inputValue.toLowerCase()
                                    return opts.filter((o) => o.DCLink.toString().includes(q) || o.Name.toLowerCase().includes(q))
                                  }}
                                  getOptionLabel={(o) => `${o.DCLink}`}
                                  value={clients.find((c) => c.DCLink === line.debtor_code) || null}
                                  onChange={(_, val) => {
                                    updateLineValue(index, 'debtor_code', val?.DCLink || '')
                                    updateLineValue(index, 'debtor_name', val?.Name || '')
                                  }}
                              ...
                              <TableCell>
                                <Autocomplete
                                  ...
                                  filterOptions={(opts, s) => {
                                    const q = s.inputValue.toLowerCase()
                                    return opts.filter((o) => o.Name.toLowerCase().includes(q) || o.DCLink.toString().includes(q))
                                  }}
                                  getOptionLabel={(o) => o.Name}
                                  value={clients.find((c) => c.DCLink === line.debtor_code) || null}
                                  onChange={(_, val) => {
                              ...
                              <TableCell>
                                <TextField
                                  size="small" value={line.description}
                                  onChange={(e) => updateLineValue(index, 'description', e.target.value)}
                                  sx={fixedField}
                                />
                              </TableCell>
                              ...
                              <TableCell>
                                <Autocomplete
                                  ...
                                  filterOptions={(opts, s) => {
                                    const q = s.inputValue.toLowerCase()
                                    return opts.filter((o) => o.Master_Sub_Account.toLowerCase().includes(q) || o.Description.toLowerCase().includes(q))
                                  }}
                                  getOptionLabel={(o) => o.Master_Sub_Account}
                                  value={accounts.find((a) => a.Master_Sub_Account === line.gl_account) || null}
                                  onChange={(_, val) => {
                                    updateLineValue(index, 'gl_account', val?.Master_Sub_Account || '')
                                    updateLineValue(index, 'account_name', val?.Description || '')
                                  }}
                                  renderInput={(params) => <TextField {...params} placeholder="Search" sx={fixedField} />}
                                />
                              </TableCell>
                              ...
                              <TableCell>
                                <Autocomplete
                                  ...
                                  filterOptions={(opts, s) => {
                                    const q = s.inputValue.toLowerCase()
                                    return opts.filter((o) => o.Description.toLowerCase().includes(q) || o.Master_Sub_Account.toLowerCase().includes(q))
                                  }}
                                  getOptionLabel={(o) => o.Description}
                                  value={accounts.find((a) => a.Master_Sub_Account === line.gl_account) || null}
                                  onChange={(_, val) => {
                              ...
                              <TableCell>
                                <TextField
                                  size="small" type="number" value={line.estimated_amount}
                                  onChange={(e) => updateLineValue(index, 'estimated_amount', e.target.value)}
                                  sx={fixedField}
                                />
                              </TableCell>
                              ...
                              <TableCell>
                                <TextField
                                  size="small" value={line.supplier_no}
                                  onChange={(e) => updateLineValue(index, 'supplier_no', e.target.value)}
                                  sx={fixedField}
                                />
                              </TableCell>
                              ...
                              <TableCell align="center">
                                <IconButton size="small" onClick={() => handleDeleteLine(index)} color="error">
                                  <DeleteIcon fontSize="small" />
                                </IconButton>
                              </TableCell>
                            </TableRow>
              ...
              {/* ── Action Buttons ──
                  Workflow: User saves as Draft OR submits (→ Submitted for HOD review)
              ── */}
              <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2 }}>
                <Button
                  variant="outlined"
                  onClick={handleCancel}
                  disabled={submitLoading}
                  sx={{ textTransform: 'none', color: '#6B7280', borderColor: '#E5E7EB', px: 4 }}
                >
                  Cancel
                </Button>

                {/* Save Draft — stays in Draft, no approval flow */}
                <Button
                  variant="outlined"
                  ...
                  sx={{
                    ...
                    borderColor: '#FF6B35',
                    px: 4,
                    '&:hover': { bgcolor: '#FFF5F0', borderColor: '#FF6B35' },
                  }}
                >
                  {submitLoading ? <CircularProgress size={20} sx={{ mr: 1 }} /> : null}
                  Save Draft
                </Button>

                {/* Submit — sends to HOD (status = Submitted) */}
                <Button
                  variant="contained"
                  onClick={handleSubmit}
                  disabled={submitLoading}
                  sx={{ textTransform: 'none', bgcolor: '#FF6B35', px: 4, '&:hover': { bgcolor: '#FF5722' } }}
                >
                  {submitLoading ? (
