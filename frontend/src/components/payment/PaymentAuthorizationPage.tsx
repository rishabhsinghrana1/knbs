...
// // //   MenuItem, Chip, IconButton, Menu,
// // //   MenuItem as MenuItemComponent, Alert,
// // // } from '@mui/material'
// // // import {
// // //   MoreVert as MoreVertIcon,
// // //   Visibility as ViewIcon,
...
// // // // ─── Constants ────────────────────────────────────────────────────────────────
// // // const years    = ['2024', '2025', '2026']
// // // const statuses = ['All', 'Draft', 'Submitted', 'FinanceApproved', 'Completed', 'Cancelled']

// // // // Roles allowed to access Payment module (as per docs)
...
// // //   try {
// // //     const raw = localStorage.getItem('user') // adjust key if different
...
// // //   const userRole: string = loggedInUser?.role_details?.role_name || ''
// // //   const hasAccess = PAYMENT_ALLOWED_ROLES.includes(userRole)

// // //   // Can delete only if Draft and role is Admin/Finance/Finance Manager/Director
// // //   const canDelete = (rowStatus: string) =>
// // //     rowStatus === 'Draft' && hasAccess
...
// // //   // ── Filter logic ───────────────────────────────────────────────────────────
// // //   const applyFilters = (yr: string, st: string, source?: any[]) => {
// // //     let result = [...(source || allData)]
...
// // //     }
// // //     if (st && st !== 'All') {
// // //       result = result.filter((row) => row.status === st)
// // //     }
// // //     setFilteredData(result)
// // //   }
...
// // //     if (row.status !== 'Draft') {
// // //       alert('Only Draft payments can be deleted.')
// // //       handleMenuClose()
// // //       return
// // //     }

// // //     if (!window.confirm(`Delete Payment No: ${row.payment_number}?`)) {
// // //       handleMenuClose()
// // //       return
// // //     }
...
// // //     try {
// // //       await fetch(
// // //         `http://localhost:8000/api/v1/payment/payment-authorization/${row.id}/`,
// // //         { method: 'DELETE' }
// // //       )
// // //       const updated = allData.filter((item) => item.id !== row.id)
// // //       setAllData(updated)
// // //       applyFilters(year, status, updated)
// // //     } catch (error) {
// // //       console.error('Delete failed:', error)
// // //     }
// // //     handleMenuClose()
...
// // //             Access Denied – Only Admin, Finance, Finance Manager or Director can access Payment module.
// // //           </Alert>
// // //         </Box>
// // //       </Box>
// // //     )
...
// // //               {/* Create button — only Draft-capable roles */}
// // //               <Button
// // //                 variant="contained"
...
// // //                   onChange={(e) => handleYearChange(e.target.value)} sx={{ minWidth: 150 }}>
// // //                   {years.map((y) => <MenuItem key={y} value={y}>{y}</MenuItem>)}
// // //                 </TextField>

// // //                 <TextField select size="small" label="Status" value={status}
// // //                   onChange={(e) => handleStatusChange(e.target.value)} sx={{ minWidth: 180 }}>
// // //                   {statuses.map((s) => <MenuItem key={s} value={s}>{s}</MenuItem>)}
// // //                 </TextField>
...
// // //             {loading && <Typography sx={{ color: '#888', mb: 2 }}>Loading...</Typography>}
...
// // //                   ) : (
// // //                     filteredData.map((row, index) => {
// // //                       const chipStyle = STATUS_COLORS[row.status] || { bg: '#F5F5F5', color: '#333' }
// // //                       return (
// // //                         <TableRow key={row.id} hover>
// // //                           <TableCell>{row.payment_number}</TableCell>
// // //                           <TableCell>{row.bank_description}</TableCell>
...
// // //                               label={row.status}
// // //                               size="small"
// // //                               sx={{ bgcolor: chipStyle.bg, color: chipStyle.color, fontWeight: 500, fontSize: '0.75rem' }}
// // //                             />
// // //                           </TableCell>
// // //                           <TableCell>
// // //                             <IconButton size="small" onClick={(e) => handleMenuOpen(e, index)}>
...
// // //               {/* Delete only shown for Draft rows */}
// // //               {selectedRow !== null && filteredData[selectedRow]?.status === 'Draft' && (
// // //                 <MenuItemComponent onClick={handleDelete} sx={{ color: '#EF5350' }}>
// // //                   <DeleteIcon sx={{ fontSize: 18, mr: 1 }} /> Delete
// // //                 </MenuItemComponent>
// // //               )}
...
// //       try {
// //         setLoading(true)
// //         const res  = await fetch('http://localhost:8000/api/v1/payment/payment-authorization/')
...
// //   const applyFilters = (yr: string, st: string, source?: any[]) => {
// //     let result = [...(source || allData)]
// //     if (yr) result = result.filter((r) => new Date(r.created_at).getFullYear().toString() === yr)
// //     if (st && st !== 'All') result = result.filter((r) => r.status === st)
// //     setFilteredData(result)
// //   }

// //   const handleYearChange   = (val: string) => { setYear(val);   applyFilters(val, status) }
// //   const handleStatusChange = (val: string) => { setStatus(val); applyFilters(year, val)   }
// //   const resetFilters = () => { setYear('2026'); setStatus('All'); applyFilters('2026', 'All') }

// //   // ── Menu ─────────────────────────────────────────────────────────────────
// //   const handleMenuOpen  = (e: React.MouseEvent<HTMLElement>, i: number) => { setAnchorEl(e.currentTarget); setSelectedRow(i) }
...
// //     if (selectedRow === null) return
// //     const row = filteredData[selectedRow]
// //     if (!window.confirm(`Delete Payment No: ${row.payment_number}?`)) { handleMenuClose(); return }
// //     try {
// //       await fetch(`http://localhost:8000/api/v1/payment/payment-authorization/${row.id}/`, { method: 'DELETE' })
// //       const updated = allData.filter((item) => item.id !== row.id)
// //       setAllData(updated)
// //       applyFilters(year, status, updated)
...
// //   // ── Selected row ka status
// //   const selectedRowStatus = selectedRow !== null ? filteredData[selectedRow]?.status : ''
...
// //               {/* Create button — sirf canCreate role ko dikhega */}
// //               {perms.canCreate && (
// //                 <Button
// //                   variant="contained"
// //                   sx={{ bgcolor: 'white', color: '#1A1A1A', textTransform: 'none', fontWeight: 600, '&:hover': { bgcolor: '#F5F5F5' } }}
...
// //                   ) : (
// //                     filteredData.map((row, index) => {
// //                       const chip = STATUS_COLORS[row.status] || { bg: '#F5F5F5', color: '#333' }
// //                       return (
// //                         <TableRow key={row.id} hover>
// //                           <TableCell>{row.payment_number}</TableCell>
// //                           <TableCell>{row.bank_description}</TableCell>
...
// //                               sx={{ bgcolor: chip.bg, color: chip.color, fontWeight: 500, fontSize: '0.75rem' }}
// //                             />
// //                           </TableCell>
// //                           <TableCell>
// //                             <IconButton size="small" onClick={(e) => handleMenuOpen(e, index)}>
...
// //               {/* Delete — sirf Draft + canDelete role */}
// //               {perms.canDelete && selectedRowStatus === 'Draft' && (
// //                 <MenuItemComponent onClick={handleDelete} sx={{ color: '#EF5350' }}>
// //                   <DeleteIcon sx={{ fontSize: 18, mr: 1 }} /> Delete
// //                 </MenuItemComponent>
// //               )}
...
//   const handleMenuOpen  = (e: React.MouseEvent<HTMLElement>, i: number) => {
...
//   // Status of the currently selected row in the context menu
//   const selectedRowStatus = selectedRow !== null ? filteredData[selectedRow]?.status : ''
...
//               {/* Show Create button only for roles that have canCreate permission */}
//               {perms.canCreate && (
//                 <Button
//                   variant="contained"
//                   sx={{ bgcolor: 'white', color: '#1A1A1A', textTransform: 'none', fontWeight: 600, '&:hover': { bgcolor: '#F5F5F5' } }}
...
//             {/* ── Context Menu — buttons shown based on role + status ── */}
//             <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleMenuClose}>
...
//               {/* Delete — only for canDelete roles and only on Draft payments */}
//               {perms.canDelete && selectedRowStatus === 'Draft' && (
//                 <MenuItemComponent onClick={handleDelete} sx={{ color: '#EF5350' }}>
//                   <DeleteIcon sx={{ fontSize: 18, mr: 1 }} /> Delete
//                 </MenuItemComponent>
...
export default function PaymentListPage() {
  ...
  // ── Filter logic ──────────────────────────────────────────────────────────
  const runFilters = (yr: string, mn: string, st: string, source?: any[]) => {
    let result = [...(source || allData)]

    if (yr) {
      result = result.filter((r) => new Date(r.created_at).getFullYear().toString() === yr)
    }
    if (mn) {
  ...
  const applyFilters = () => runFilters(year, month, status)
  ...
  const handleView = () => {
    ...
    router.push(`/payament/view/${selectedRow.id}`)
    handleMenuClose()
  }

  const handleEdit = () => {
    if (!selectedRow) return
    router.push(`/payament/edit/${selectedRow.id}`)
    handleMenuClose()
  }

  const handleDelete = async () => {
    if (!selectedRow) return
    if (!window.confirm(`Delete Payment No: ${selectedRow.payment_number}?`)) {
      handleMenuClose()
      return
    }
    try {
      await fetch(
        `http://localhost:8000/api/v1/payment/payment-authorization/${selectedRow.id}/`,
        { method: 'DELETE' }
      )
      const updated = allData.filter((item) => item.id !== selectedRow.id)
      setAllData(updated)
      runFilters(year, month, status, updated)
    } catch (e) {
      console.error('Delete failed:', e)
    }
    handleMenuClose()
  ...
  const canEditRow = (row: any) => perms.canCreate && row?.status === 'Draft'
  const canDeleteRow = (row: any) => perms.canDelete && row?.status === 'Draft'
          ...
          <Paper elevation={0} sx={{ p: 3, borderRadius: 2, boxShadow: '0 4px 20px rgba(0,0,0,0.05)' }}>
            ...
            <Box sx={{
              ...
              <Box>
                ...
                <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.9)' }}>
                  {user?.name ? `Logged in as: ${user.name}` : 'Manage and process payment batches'}
                </Typography>
              </Box>
                ...
                <TableBody>
                  {loading ? (
                    ...
                    filteredData.map((row) => {
                      const chip = STATUS_COLORS[row.status] || { bg: '#F5F5F5', color: '#333' }
                      const totalAmt = row.lines?.reduce((s: number, l: any) => s + Number(l.amount || 0), 0) || 0
                      return (
                        <TableRow key={row.id} hover>
                          <TableCell>{row.payment_number}</TableCell>
                          <TableCell>{row.bank_description || '—'}</TableCell>
            ...
            <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleMenuClose}>
              ...
              {canEditRow(selectedRow) && (
                <MenuItemComponent onClick={handleEdit}>
                  <EditIcon sx={{ fontSize: 18, mr: 1 }} /> Edit
                </MenuItemComponent>
              )}

              {canDeleteRow(selectedRow) && (
                <MenuItemComponent onClick={handleDelete} sx={{ color: '#EF5350' }}>
                  <DeleteIcon sx={{ fontSize: 18, mr: 1 }} /> Delete
                </MenuItemComponent>
              )}
