...
const PENDING_STATUSES  = ['Draft', 'Pending_HOD', 'HODApproved', 'Pending_Finance']
const APPROVED_STATUSES = ['FinanceApproved', 'FMApproved', 'Completed']
const REJECTED_STATUSES = ['Rejected']
const ONHOLD_STATUSES   = ['Surrendered']

const departments = ['Finance Department', 'HR Department', 'Marketing Department', 'Software Department']
const years       = ['2024', '2025', '2026']
const months      = [
...
export default function DashboardPage() {
  ...
  const [allImprests,  setAllImprests]  = useState<any[]>([])
  ...
  useEffect(() => {
    const fetchImprests = async () => {
      try {
        setLoading(true)
        const res  = await fetch(`${API_BASE_URL}/imprest/`)
        const data = await res.json()
        setAllImprests(Array.isArray(data) ? data : [])
      } catch (e) {
        console.error('Failed to fetch imprests:', e)
      } finally {
        setLoading(false)
      }
    }
  ...
  const applyDateFilter = (data: any[]) => {
    ...
    if (selectedYear) {
      result = result.filter(
        (r) => new Date(r.created_on).getFullYear().toString() === selectedYear
      )
    }
    if (selectedMonth) {
    ...
    if (selectedDept) {
      result = result.filter(
        (r) => r.department_name?.toLowerCase().includes(selectedDept.toLowerCase())
      )
    }
    return result
  }
  ...
  // ── Derived data ───────────────────────────────────────────────────────────
  const filtered = applyDateFilter(allImprests)

  // My Pending — logged-in user ke imprests jo pending hain
  const myPending = loggedInUser
    ? filtered
        .filter(
          (r) =>
            r.user_requested === loggedInUser.user_id &&
            PENDING_STATUSES.includes(r.status)
        )
        .slice(0, 5)
    : []

  // Department Pending — same department ke pending imprests
  const deptPending = loggedInUser
    ? filtered
        .filter(
          (r) =>
            r.department === loggedInUser.department_id &&
            PENDING_STATUSES.includes(r.status)
        )
        .slice(0, 5)
    : []

  // Donut chart counts — from all imprests (no date filter for chart)
  const chartData = {
    approved: allImprests.filter((r) => APPROVED_STATUSES.includes(r.status)).length,
    pending:  allImprests.filter((r) => PENDING_STATUSES.includes(r.status)).length,
    onHold:   allImprests.filter((r) => ONHOLD_STATUSES.includes(r.status)).length,
    rejected: allImprests.filter((r) => REJECTED_STATUSES.includes(r.status)).length,
  }
  const total = chartData.approved + chartData.pending + chartData.onHold + chartData.rejected || 1

  // Donut SVG math
  const CIRCUMFERENCE = 2 * Math.PI * 60 // r=60 → ~377
  const approvedDash  = (chartData.approved / total) * CIRCUMFERENCE
  const pendingDash   = (chartData.pending  / total) * CIRCUMFERENCE
  const onHoldDash    = (chartData.onHold   / total) * CIRCUMFERENCE
  const rejectedDash  = (chartData.rejected / total) * CIRCUMFERENCE

  // ── Filter dialog ──────────────────────────────────────────────────────────
  const handleSearch = () => setFilterOpen(false)
  ...
  return (
    <Box sx={{ display: 'flex', height: '100vh', overflow: 'hidden', bgcolor: '#F5F7FA' }}>
            ...
            {loading ? (
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} lg={8}>
                    ...
                    <Box sx={{ p: 2.5, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <Typography variant="subtitle1" sx={{ fontWeight: 600, fontSize: '1.125rem' }}>
                        ...
                        <Chip
                          ...
                          size="small"
                          sx={{ ml: 1, bgcolor: '#FFF3E0', color: '#FF6B35', fontWeight: 700, fontSize: '0.75rem' }}
                        />
                      </Typography>
                      <Box sx={{ display: 'flex', gap: 1 }}>
                        <Button
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {myPending.length === 0 ? (
                            ...
                            <TableRow
                              key={row.imprest_id} hover
                              sx={{ cursor: 'pointer', '&:last-child td': { border: 0 } }}
                              onClick={() => router.push(`/imprest/view/${row.imprest_id}`)}
                            >
                              <TableCell sx={{ fontSize: '0.875rem', py: 2.5 }}>{row.request_no}</TableCell>
                              <TableCell sx={{ fontSize: '0.875rem', py: 2.5 }}>{fmtDate(row.created_on)}</TableCell>
                              <TableCell sx={{ fontSize: '0.875rem', py: 2.5 }}>{row.department_name}</TableCell>
                              <TableCell sx={{ py: 2.5 }}>{getStatusChip(row.status)}</TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
                      ...
                      <Table>
                        <TableHead sx={{ bgcolor: '#F9FAFB' }}>
                          <TableRow>
                            ...
                            <TableCell sx={{ fontWeight: 600, color: '#6B7280', fontSize: '0.875rem', py: 2 }}>Req. Date</TableCell>
                            <TableCell sx={{ fontWeight: 600, color: '#6B7280', fontSize: '0.875rem', py: 2 }}>Department</TableCell>
                            <TableCell sx={{ fontWeight: 600, color: '#6B7280', fontSize: '0.875rem', py: 2 }}>Requested By</TableCell>
                            <TableCell sx={{ fontWeight: 600, color: '#6B7280', fontSize: '0.875rem', py: 2 }}>Status</TableCell>
                          </TableRow>
                        </TableHead>
                        <TableBody>
                          {deptPending.length === 0 ? (
                            <TableRow>
                              <TableCell colSpan={5} align="center" sx={{ py: 4, color: '#9CA3AF' }}>
                                No department pending approvals
                              </TableCell>
                            </TableRow>
                          ) : deptPending.map((row) => (
                            <TableRow
                            ...
                            >
                              ...
                              <TableCell sx={{ fontSize: '0.875rem', py: 2.5 }}>{fmtDate(row.created_on)}</TableCell>
                              <TableCell sx={{ fontSize: '0.875rem', py: 2.5 }}>{row.department_name}</TableCell>
                              <TableCell sx={{ fontSize: '0.875rem', py: 2.5 }}>{row.user_requested_name}</TableCell>
                              <TableCell sx={{ py: 2.5 }}>{getStatusChip(row.status)}</TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
                  ...
                  {/* Budget Utilization — placeholder jab tak API na aaye */}
                  <Paper elevation={0} sx={{ borderRadius: 2, overflow: 'hidden' }}>
                    <Box sx={{ p: 2.5, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <Typography variant="subtitle1" sx={{ fontWeight: 600, fontSize: '1.125rem' }}>
                        Budget Utilization
                    ...
                    <Box sx={{ p: 4, textAlign: 'center', color: '#9CA3AF' }}>
                      <Typography variant="body2">
                        Budget data coming soon. Connect budget API to enable this section.
                      </Typography>
                    </Box>
                  </Paper>
                ...
                <Grid item xs={12} lg={4}>
                  ...
                  <Paper elevation={0} sx={{ p: 2, mb: 3, borderRadius: 2 }}>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', mb: 2 }}>
                      <svg width="180" height="180" viewBox="0 0 180 180">
                        ...
                        <circle
                          cx="90" cy="90" r="60" fill="none"
                          stroke="#EF5350" strokeWidth="28"
                          strokeDasharray={`${rejectedDash} ${CIRCUMFERENCE}`}
                          strokeDashoffset={-(approvedDash + pendingDash + onHoldDash)}
                          transform="rotate(-90 90 90)"
                        />
                        {/* Center total */}
                        <text x="90" y="86" textAnchor="middle" fontSize="18" fontWeight="700" fill="#111827">
                          {total}
                        </text>
                        <text x="90" y="102" textAnchor="middle" fontSize="10" fill="#9CA3AF">
                          Total
                        </text>
                      </svg>
                    </Box>
                    ...
                    <Grid container spacing={1.5}>
                      {[
                        { label: 'Approved', count: chartData.approved, color: '#4CAF50' },
                        { label: 'Pending',  count: chartData.pending,  color: '#5B7FFF' },
                        { label: 'On Hold',  count: chartData.onHold,   color: '#FFA726' },
                        { label: 'Rejected', count: chartData.rejected, color: '#EF5350' },
                      ].map((item) => (
                  ...
                  <Paper elevation={0} sx={{ p: 2, borderRadius: 2 }}>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                      {[
                        { label: 'Imprest Request',      icon: <DocumentIcon sx={{ color: 'white', fontSize: '1.1rem' }} />, route: '/imprest' },
                        { label: 'Payment Authorization', icon: <PaymentIcon  sx={{ color: 'white', fontSize: '1.1rem' }} />, route: '/payament/add' },
                      ].map((action) => (
                        <Card
                          key={action.label}
                          elevation={0}
                          ...
                          sx={{
                            bgcolor: '#FFF4F0', borderRadius: 2, cursor: 'pointer',
                            transition: 'all 0.2s',
                            '&:hover': {
                              bgcolor: '#FFE8DF',
                              transform: 'translateY(-2px)',
      ...
      >
        <DialogTitle sx={{ pb: 1 }}>
          <Typography variant="h6" sx={{ fontWeight: 600 }}>Filter Results</Typography>
        </DialogTitle>
        <DialogContent sx={{ pt: 2 }}>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, pt: 1 }}>
            <TextField
        ...
        <DialogActions sx={{ px: 3, pb: 3 }}>
          ...
          <Button
            onClick={handleSearch}
            variant="contained"
            sx={{ textTransform: 'none', bgcolor: '#FF6B35', fontWeight: 600, px: 3, '&:hover': { bgcolor: '#FF5722' } }}
          >
            Search Results
          </Button>
        </DialogActions>
